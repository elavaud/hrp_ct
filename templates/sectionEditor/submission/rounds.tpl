...
<table width="100%" class="data">
	<tr valign="top">
		...
		<td class="value" width="80%">
			...
			{if $reviewFile}
				<a href="{url op="downloadFile" path=$articleId|to_array:$reviewFile->getFileId():$reviewFile->getRevision()}" class="file">{$reviewFile->getFileName()|escape}</a>&nbsp;&nbsp;{$reviewFile->getDateModified()|date_format:$dateFormatShort}
			{else}
...
<table width="100%" class="listing">
	...
	{if $reviewFormResponses[$reviewId]}
		<tr valign="top">
			...
			<td>
				<a href="javascript:openComments('{url op="viewReviewFormResponse" path=$articleId|to_array:$reviewAssignment->getId()}');" class="icon">{icon name="letter"}</a>
			</td>
		</tr>
	{/if}
	<tr valign="top">
		...
		<td colspan="4">
			{if $reviewAssignment->getMostRecentPeerReviewComment()}
				...
				<a href="javascript:openComments('{url op="viewPeerReviewComments" path=$articleId|to_array:$reviewAssignment->getId() anchor=$comment->getId()}');" class="icon">{icon name="comment"}</a> {$comment->getDatePosted()|date_format:$dateFormatShort}
			{else}
				<a href="javascript:openComments('{url op="viewPeerReviewComments" path=$articleId|to_array:$reviewAssignment->getId()}');" class="icon">{icon name="comment"}</a>{translate key="common.noComments"}
			{/if}
		</td>
	</tr>
 	<tr valign="top">
				...
				<tr valign="top">
					<td valign="middle">
						<form name="authorView{$reviewAssignment->getId()}" method="post" action="{url op="makeReviewerFileViewable"}">
							<a href="{url op="downloadFile" path=$articleId|to_array:$reviewerFile->getFileId():$reviewerFile->getRevision()}" class="file">{$reviewerFile->getFileName()|escape}</a>&nbsp;&nbsp;{$reviewerFile->getDateModified()|date_format:$dateFormatShort}
...
<table class="data" width="100%">
	...
	<tr valign="top">
		...
		<td class="value" width="80%">
			...
			{if $submission->getMostRecentEditorDecisionComment()}
				...
				<a href="javascript:openComments('{url op="viewEditorDecisionComments" path=$articleId anchor=$comment->getId()}');" class="icon">{icon name="comment"}</a> {$comment->getDatePosted()|date_format:$dateFormatShort}
			{else}
				<a href="javascript:openComments('{url op="viewEditorDecisionComments" path=$articleId}');" class="icon">{icon name="comment"}</a>{translate key="common.noComments"}
			{/if}
		</td>
	</tr>
	{foreach from=$authorFiles item=authorFile key=key}
		<tr valign="top">
			...
			{/if}
			<td width="80%" class="value"><a href="{url op="downloadFile" path=$articleId|to_array:$authorFile->getFileId():$authorFile->getRevision()}" class="file">{$authorFile->getFileName()|escape}</a>&nbsp;&nbsp;{$authorFile->getDateModified()|date_format:$dateFormatShort}</td>
		</tr>
	{foreachelse}
		<tr valign="top">
	...
	{foreach from=$editorFiles item=editorFile key=key}
		<tr valign="top">
			...
			<td width="30%"><a href="{url op="downloadFile" path=$articleId|to_array:$editorFile->getFileId():$editorFile->getRevision()}" class="file">{$editorFile->getFileName()|escape}</a>&nbsp;&nbsp;{$editorFile->getDateModified()|date_format:$dateFormatShort}</td>
		</tr>
	{foreachelse}
		<tr valign="top">
